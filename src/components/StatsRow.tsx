...
export function StatsRow() {
  ...
  const today = flights.filter((f) => f.date === todaySGT);

  const total = today.length;
  const onTime = today.filter((f) => f.status === "On Time").length;
  const delayed = today.filter((f) => f.status === "Delayed").length;
  const cancelled = today.filter((f) => f.status === "Cancelled").length;

  const stats = [
    { label: "Today's Transfers", value: total, icon: PlaneTakeoff, color: "text-blue-400", bg: "bg-blue-400/10", border: "border-blue-400/20" },
